...
def get_successful_test(classifier, x_test, y_test):
  ...
  def generalized_gamma(x, mu = -2.6, a = 1, b = 3, c = 3, sf =1.17):
    x = tf.math.divide(x-mu, b) # normalizing 
    return tf.math.divide(stats.gengamma.pdf(x, a, c), sf)

# divide by special.gamma(a)
def gamma_derivative(x, mu = -2.6, a = 1, b = 3, c = 3, sf =1.17):
    x = tf.math.divide(x-mu, b) # normalizing 
    der = tf.math.divide(c*tf.math.exp(-x**c)*((a*c)+c*(-x)**c-1)*x**(a*c-2), math.gamma(a))
    return tf.math.divide(der, sf)

@tf.custom_gradient
def gamma_activate(x, mu = -2.6, a = 1, b = 3, c = 3, sf =1.17):

    def grad(dy):
      return gamma_derivative(x, mu, a, b, sf) * dy

    res = generalized_gamma(x, mu, a, b, sf)
    res = tf.cast(res, tf.float32)
    #res = tf.convert_to_tensor(res, np.float32)
    res 
    return res, grad
  
model = define_model(gamma_activate)
